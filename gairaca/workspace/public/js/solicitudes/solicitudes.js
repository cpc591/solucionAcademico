var app = angular.module("appSolicitud", ['solicitudService', 'ngMaterial', 'ngMessages', 'lfNgMdFileInput', 'ng.ckeditor', 'ui.select', 'ngSanitize']);
...
app.controller('responderCtrl', function($scope, solicitudServi) {
    ...
    $scope.$watch('idSolicitud', function() {
        if ($scope.idSolicitud) {
            ...
            solicitudServi.obtenerSolicitudResponder($scope.idSolicitud).then(function(dato) {
                ...
                $scope.respuesta.correosDependencias = dato.selectDependencias;
                $scope.respuesta.correosDependenciasAdministrativas = dato.selectDependenciasAdministrativas;
                console.log($scope.respuesta);
                $scope.multimedias_respuesta = dato.multimedias_respuesta;
                $scope.dependencias = dato.dependencias;
                $scope.dependenciasAdministrativas = dato.dependenciasAdministrativas;
                $("#load2").addClass("hidden");
    ...
    $scope.crear_respuesta = function(dato) {
        ...
        for (contenido in $scope.respuesta) {
            ...
            if (contenido == 'correosDependencias' || contenido == 'correosDependenciasAdministrativas') {
                for (j = 0; j < $scope.respuesta[contenido].length; j++) {
                    fd.append(contenido + '[]', $scope.respuesta[contenido][j]);
                }
            } else if ($scope.respuesta[contenido] != null && $scope.respuesta[contenido] != "") {
    ...
    $scope.crear_borrador = function(dato) {
        ...
        for (contenido in $scope.respuesta) {
            ...
            if (contenido == 'correosDependencias' || contenido == 'correosDependenciasAdministrativas') {
                for (var j = 0; j < $scope.respuesta[contenido].length; j++) {
                    fd.append(contenido + '[]', $scope.respuesta[contenido][j]);
                }
            } else if ($scope.respuesta[contenido] != null && $scope.respuesta[contenido] != "") {
                fd.append(contenido, $scope.respuesta[contenido])
